export const detectarAlertaRoja = () => {
  ...
  recientes.forEach(r => {
    ...
    porAprendiz[nombre] = porAprendiz[nombre] || new Set();
    porAprendiz[nombre].add(r.instructor || 'anonimo');
  });

  const alertas = Object.entries(porAprendiz).filter(([_, instructores]) => instructores.size >= 3);
  return alertas.map(([nombre]) => ({
    tipo: 'Roja',
    aprendiz: nombre,
...
export const detectarAlertaAmarilla = () => {
  ...
  const alertas = Object.entries(porAprendiz).filter(([_, fecha]) => {
    const dias = (ahora - fecha) / (1000 * 60 * 60 * 24);
    return dias >= 28;
  });
